...
func ExampleOpenHandleWithOpts_one() {
	...
	h, err = snf.OpenHandleWithOpts(1)
	if err != nil {
		return
	}
	defer h.Close()
...
func ExampleOpenHandleWithOpts_two() {
	if err := snf.Init(); err != nil {
		return
	}

	// sample default handler
	h, err := snf.OpenHandleWithOpts(0)
	if err != nil {
		return
	}
	defer h.Close()
